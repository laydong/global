// Package logger is a port of File-RotateLogs from Perl
...
import (
	"fmt"
	fileutil2 "github.com/climber-dong/global/core/logx/logger/fileutil"
	"github.com/climber-dong/global/core/logx/logger/strftime"
...
// New creates a new RotateLogs object. A logger filename pattern
...
func New(p string, options ...Option) (*RotateLogs, error) {
	...
	// 获取初始化位移
	generation := 0
	baseFn := fileutil2.GenerateFn(pattern, clock, rotationTime)
	...
	rl.outFh = fileutil2.GenerateBuffIO()
	return rl, nil
}
...
func (rl *RotateLogs) getWriterNolock(bailOnRotateFail, useGenerationalNames bool) error {
	...
	// to logger to, which may be newer than rl.currentFilename
	baseFn := fileutil2.GenerateFn(rl.pattern, rl.clock, rl.rotationTime)
	filename := baseFn
	var forceNewFile bool
...
// Rotate forcefully rotates the logger files. If the generated file name
// clash because file already exists, a numeric suffix of the form
// ".1", ".2", ".3" and so forth are appended to the end of the logger file
//
// Thie method can be used in conjunction with a signal handler so to
// emulate servers that generate new logger files when they receive a
// SIGHUP
